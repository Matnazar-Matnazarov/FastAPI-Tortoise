...
@router.post("/{comment_id}", response_model=CommentLikes, status_code=status.HTTP_201_CREATED)
...
async def read_comment_like(comment_like_id: int, current_user: User = Depends(get_current_user)):
    db_comment_like = await get_comment_like(comment_like_id)
    if db_comment_like is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Comment like not found")
